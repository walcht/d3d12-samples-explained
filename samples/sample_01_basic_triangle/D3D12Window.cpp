...
void D3D12Window::LoadAssets() {
   ...
   *
   * This whole thing is confusing, just read-along until we reach the texture
   * chapter where Shader resources are actually used.
   *
   * Here, the *default* root signature is created since our Shader still
   * doesn't access any resources whatsover. In future examples (e.g.,
   * basic_texture sample), the utility of a root signature will become clearer.
   */
  {
  ...
  // Create the pipeline state, which includes compiling and loading shaders.
  {
    ...
    auto s = GetAssetFullPath(L"shaders_VSMain.cso");
...
void D3D12Window::PopulateGraphicsCmdList() {
  ...
  m_graphics_cmd_list->ClearRenderTargetView(rtv_handle, clear_color, 0,
                                             nullptr);
  m_graphics_cmd_list->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
  m_graphics_cmd_list->IASetVertexBuffers(0, 1, &m_vertex_buffer_view);
  m_graphics_cmd_list->DrawInstanced(3, 1, 0, 0);
