...
D3D12Window::D3D12Window(UINT width, UINT height, std::wstring name)
    : Win32BaseWindow<D3D12Window>(), m_width(width), m_height(height),
      ...
      m_rtv_descriptor_size(0), m_constant_buffer_data{} {
...
void D3D12Window::InitPipeline() {
  ...
  /* 6. Create the descriptor heaps. */
  {
    ...
    ThrowIfFailed(m_device->CreateDescriptorHeap(&cbv_heap_desc, IID_PPV_ARGS(&m_cbv_heap)));
  }
...
void D3D12Window::LoadAssets() {
   ...
   *
   * Since we are now using a constant buffer resource in our Shader, we have to
   * set the root signature for this usage:
   *    1. Check if the device supports root signature 1.1 otherwise fallback
   *       to 1.0
   *    2. Create descriptor ranges. Initialize just one element with our CBV
   *       (i.e., )
   *
   */
  {
    ...
    root_parameters[0].InitAsDescriptorTable(_countof(ranges), ranges, D3D12_SHADER_VISIBILITY_VERTEX);
  ...
  /* Create the cbuffer */
  {
      assert(sizeof(SceneConstantBuffer) % 256 == 0);
      const auto heap_props = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
      const auto resource_desc = CD3DX12_RESOURCE_DESC::Buffer(sizeof(SceneConstantBuffer));
      ThrowIfFailed(m_device->CreateCommittedResource(&heap_props,
            D3D12_HEAP_FLAG_NONE,
            &resource_desc,
            D3D12_RESOURCE_STATE_GENERIC_READ,
            nullptr,
      ...
      // cbuffers are not accessed directly via a shader but rather through a
      // view. Create said view in the UAV/SRV/CBV descriptor heap
      D3D12_CONSTANT_BUFFER_VIEW_DESC cbv_desc = {};
      cbv_desc.BufferLocation = m_constant_buffer->GetGPUVirtualAddress();
      cbv_desc.SizeInBytes = sizeof(SceneConstantBuffer);
      m_device->CreateConstantBufferView(&cbv_desc, m_cbv_heap->GetCPUDescriptorHandleForHeapStart());

      // since this buffer data changes each frame, we need to map it and keep
      // it mapped until this is no longer needed (i.e., app exit). Apparently,
      // keeping resources mapped for a very long lifetime is ok(?).
      CD3DX12_RANGE read_range(0, 0);
      ThrowIfFailed(m_constant_buffer->Map(0, &read_range, reinterpret_cast<void**>(&m_pcbv_data_begin)));
      memcpy(m_pcbv_data_begin, &m_constant_buffer_data, sizeof(SceneConstantBuffer)); 
  }
...
void D3D12Window::OnUpdate() {
  
  {
    ...
    memcpy(m_pcbv_data_begin, &m_constant_buffer_data, sizeof(m_constant_buffer_data));
  }
...
void D3D12Window::PopulateGraphicsCmdList() {
  ...
  ID3D12DescriptorHeap* ppheaps[1] = { m_cbv_heap.Get() };
  m_graphics_cmd_list->SetDescriptorHeaps(_countof(ppheaps), ppheaps);
  m_graphics_cmd_list->SetGraphicsRootDescriptorTable(0, m_cbv_heap->GetGPUDescriptorHandleForHeapStart());
